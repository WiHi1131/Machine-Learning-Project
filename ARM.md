...
Rules are discovered truths about the dataset in question. They are typically written $$X \rightarrow Y$$, where $$X$$ and $$Y$$ are different itemsets. The rule above would indicate that a customer who bought item $$X$$ is likely to buy item $$Y$$. The threshold for determining whether a likelihood becomes a rule is arbitrarily chosen. 
...
From each frequent itemset that the algorithm finds, we create rules. For example, if {milk, bread} is frequent, we consider rules such as $${milk} \rightarrow {bread}$$ and $${bread} \rightarrow {milk}$$, as long as they meet the minimum confidence threshold. 
